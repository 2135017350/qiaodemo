const express = require('express');
const mysql = require('mysql2');
const bodyParser = require('body-parser');
const cors = require('cors');
const path = require('path');

const app = express();
const PORT = 3000;

// Middleware
app.use(cors());
app.use(bodyParser.json());
app.use(express.static(path.join(__dirname, 'public')));

// Database Connection (without database name first)
const db = mysql.createConnection({
    host: 'localhost',
    user: 'root',
    password: 'root'
});

// Initialize Database and Tables
function initDatabase() {
    return new Promise((resolve, reject) => {
        // Create database if not exists
        db.query('CREATE DATABASE IF NOT EXISTS bridge_rhyme', (err) => {
            if (err) {
                console.error('创建数据库失败:', err);
                reject(err);
                return;
            }
            console.log('✅ 数据库 bridge_rhyme 已就绪');

            // Use the database
            db.query('USE bridge_rhyme', (err) => {
                if (err) {
                    reject(err);
                    return;
                }

                // Create users table
                db.query(`
                    CREATE TABLE IF NOT EXISTS users (
                        id INT AUTO_INCREMENT PRIMARY KEY,
                        username VARCHAR(50) NOT NULL UNIQUE,
                        email VARCHAR(100) NOT NULL UNIQUE,
                        password VARCHAR(255) NOT NULL,
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                    )
                `, (err) => {
                    if (err) console.error('创建users表失败:', err);
                });

                // Create bridges table
                db.query(`
                    CREATE TABLE IF NOT EXISTS bridges (
                        id INT AUTO_INCREMENT PRIMARY KEY,
                        name VARCHAR(100) NOT NULL COMMENT '桥梁名称',
                        name_en VARCHAR(100) COMMENT '英文名称',
                        location VARCHAR(200) COMMENT '地理位置',
                        province VARCHAR(50) COMMENT '省份',
                        city VARCHAR(50) COMMENT '城市',
                        description TEXT COMMENT '桥梁描述',
                        history TEXT COMMENT '历史背景',
                        construction_year INT COMMENT '建造年份',
                        length DECIMAL(10,2) COMMENT '长度(米)',
                        width DECIMAL(10,2) COMMENT '宽度(米)',
                        span DECIMAL(10,2) COMMENT '主跨跨度(米)',
                        bridge_type VARCHAR(50) COMMENT '桥梁类型',
                        latitude DECIMAL(10,6) COMMENT '纬度',
                        longitude DECIMAL(10,6) COMMENT '经度',
                        image_url VARCHAR(500) COMMENT '图片URL',
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                    )
                `, (err) => {
                    if (err) console.error('创建bridges表失败:', err);
                });

                // Check if bridges data exists
                db.query('SELECT COUNT(*) as count FROM bridges', (err, results) => {
                    if (err) {
                        console.error('检查桥梁数据失败:', err);
                        resolve();
                        return;
                    }

                    // Insert sample data if empty
                    if (results[0].count === 0) {
                        const insertSQL = `
                            INSERT INTO bridges (name, name_en, location, province, city, description, history, construction_year, length, width, span, bridge_type, latitude, longitude) VALUES
                            ('赵州桥', 'Zhaozhou Bridge', '河北省石家庄市赵县', '河北省', '石家庄市', '世界上现存最早、保存最好的敞肩式石拱桥', '建于隋朝大业年间(605-618年)，由李春设计建造', 605, 50.82, 9.6, 37.4, '石拱桥', 37.7697, 114.7764),
                            ('卢沟桥', 'Lugou Bridge', '北京市丰台区', '北京市', '北京市', '北京市现存最古老的石造联拱桥', '始建于金大定二十九年(1189年)，明正统九年(1444年)重修', 1189, 266.5, 9.3, 21.5, '石拱桥', 39.8517, 116.2233),
                            ('南京长江大桥', 'Nanjing Yangtze River Bridge', '江苏省南京市', '江苏省', '南京市', '中国自行设计建造的第一座双层式铁路、公路两用桥梁', '1968年建成通车，是20世纪60年代中国经济建设的重要成就', 1968, 4589, 19.5, 160, '钢梁桥', 32.0146, 118.7391),
                            ('港珠澳大桥', 'Hong Kong-Zhuhai-Macao Bridge', '广东省珠海市', '广东省', '珠海市', '世界上最长的跨海大桥', '2009年动工，2018年正式通车，总长55公里', 2018, 55000, 33.1, 458, '斜拉桥', 22.2855, 113.5767),
                            ('杭州湾跨海大桥', 'Hangzhou Bay Bridge', '浙江省嘉兴市', '浙江省', '嘉兴市', '世界第三长跨海大桥', '2008年建成通车，连接嘉兴与宁波', 2008, 36000, 33, 80, '斜拉桥', 30.5167, 121.0833),
                            ('泸定桥', 'Luding Bridge', '四川省甘孜藏族自治州泸定县', '四川省', '甘孜藏族自治州', '中国著名的红色桥梁，红军长征飞夺泸定桥', '始建于清康熙四十四年(1705年)', 1705, 103, 2.8, 100, '铁索桥', 29.9152, 102.2493),
                            ('五亭桥', 'Wuting Bridge', '江苏省扬州市瘦西湖', '江苏省', '扬州市', '中国最美的桥梁之一，仿北京北海五龙亭', '建于清乾隆二十二年(1757年)', 1757, 57, 6.6, 7, '石拱桥', 32.4217, 119.4283),
                            ('程阳永济桥', 'Yongji Bridge', '广西壮族自治区柳州市三江侗族自治县', '广西壮族自治区', '柳州市', '中国保存最完好、规模最大的侗族风雨桥', '建于民国九年(1920年)', 1920, 64.4, 3.4, 20, '侗族风雨桥', 25.9333, 109.6),
                            ('武汉长江大桥', 'Wuhan Yangtze River Bridge', '湖北省武汉市', '湖北省', '武汉市', '万里长江第一桥', '1957年建成通车', 1957, 1670, 22.5, 128, '钢梁桥', 30.5456, 114.2693),
                            ('重庆千厮门嘉陵江大桥', 'Qiansimen Bridge', '重庆市渝中区', '重庆市', '重庆市', '重庆嘉陵江上的著名桥梁', '2015年建成', 2015, 720, 26, 240, '斜拉桥', 29.5536, 106.5872),
                            ('颐和园十七孔桥', 'Seventeen-Arch Bridge', '北京市海淀区颐和园', '北京市', '北京市', '颐和园中著名的石桥', '清乾隆年间建造', 1750, 150, 8, 8, '石拱桥', 39.9989, 116.2756),
                            ('都江堰安澜桥', 'Anlan Bridge', '四川省成都市都江堰市', '四川省', '成都市', '都江堰景区著名索桥', '宋代的索桥，清代修复', 970, 240, 2.4, 60, '竹索桥', 31.0022, 103.6122),
                            ('晋江安平桥', 'Anping Bridge', '福建省泉州市晋江市', '福建省', '泉州市', '中国古代最长的石桥', '南宋绍兴八年(1138年)建造', 1138, 2070, 5, 10, '石板桥', 24.7822, 118.5486),
                            ('广州海珠桥', 'Haizhu Bridge', '广东省广州市越秀区', '广东省', '广州市', '广州市中心跨越珠江的桥梁', '1933年建成，是广州第一座跨江桥', 1933, 356, 18.5, 56, '钢梁桥', 23.1123, 113.2589),
                            ('上海外白渡桥', 'Waibaidu Bridge', '上海市黄浦区', '上海市', '上海市', '中国第一座全钢结构铆接桥梁', '1907年建成，是上海标志性建筑', 1907, 106.7, 18.4, 52, '钢梁桥', 31.2500, 121.4917)
                        `;
                        
                        db.query(insertSQL, (err) => {
                            if (err) {
                                console.error('插入桥梁数据失败:', err);
                            } else {
                                console.log('✅ 15座中国著名桥梁数据已导入');
                            }
                            resolve();
                        });
                    } else {
                        console.log(`✅ 桥梁数据已存在 (${results[0].count} 条)`);
                        resolve();
                    }
                });
            });
        });
    });
}

// ============ 桥梁API ============

app.get('/api/bridges', (req, res) => {
    const query = 'SELECT * FROM bridges ORDER BY id';
    db.query(query, (err, results) => {
        if (err) {
            console.error('获取桥梁失败:', err);
            return res.status(500).json({ error: '获取桥梁失败' });
        }
        res.json(results);
    });
});

app.get('/api/bridges/:id', (req, res) => {
    const query = 'SELECT * FROM bridges WHERE id = ?';
    db.query(query, [req.params.id], (err, results) => {
        if (err) return res.status(500).json({ error: '获取桥梁失败' });
        if (results.length === 0) return res.status(404).json({ error: '桥梁不存在' });
        res.json(results[0]);
    });
});

app.get('/api/bridges/province/:province', (req, res) => {
    const query = 'SELECT * FROM bridges WHERE province = ?';
    db.query(query, [req.params.province], (err, results) => {
        if (err) return res.status(500).json({ error: '查询失败' });
        res.json(results);
    });
});

app.get('/api/bridges/type/:type', (req, res) => {
    const query = 'SELECT * FROM bridges WHERE bridge_type = ?';
    db.query(query, [req.params.type], (err, results) => {
        if (err) return res.status(500).json({ error: '查询失败' });
        res.json(results);
    });
});

app.get('/api/bridges/search', (req, res) => {
    const keyword = req.query.keyword || '';
    const query = 'SELECT * FROM bridges WHERE name LIKE ? OR name_en LIKE ? OR location LIKE ?';
    const likePattern = `%${keyword}%`;
    db.query(query, [likePattern, likePattern, likePattern], (err, results) => {
        if (err) return res.status(500).json({ error: '搜索失败' });
        res.json(results);
    });
});

// ============ 聊天API ============

let conversationHistory = [];

app.post('/api/chat', async (req, res) => {
    const { message } = req.body;
    
    if (!message) {
        return res.status(400).json({ message: '请输入消息' });
    }

    const bridges = await new Promise((resolve, reject) => {
        db.query('SELECT * FROM bridges', (err, results) => {
            if (err) reject(err);
            else resolve(results);
        });
    });

    const response = {};
    const answer = processLocally(message, bridges);
    response.message = answer;
    response.action = determineAction(message, bridges);

    conversationHistory.push({ role: 'user', content: message });
    conversationHistory.push({ role: 'assistant', content: response.message });
    if (conversationHistory.length > 10) {
        conversationHistory = conversationHistory.slice(-10);
    }

    res.json(response);
});

app.post('/api/chat/clear', (req, res) => {
    conversationHistory = [];
    res.json({ success: true });
});

function processLocally(userMessage, bridges) {
    const lowerMsg = userMessage.toLowerCase();
    
    for (const bridge of bridges) {
        const name = bridge.name.toLowerCase();
        const nameEn = bridge.name_en ? bridge.name_en.toLowerCase() : '';
        
        if (lowerMsg.includes(name) || lowerMsg.includes(nameEn) || 
            lowerMsg.includes(bridge.province) || lowerMsg.includes(bridge.city)) {
            return `好的！让我带你看看${bridge.name}，这座桥位于${bridge.province}${bridge.city}，${bridge.description || ''}`;
        }
    }
    
    if (lowerMsg.includes('最长') || lowerMsg.includes('最大')) {
        return '中国最长的桥是港珠澳大桥，全长55公里，是世界上最长的跨海大桥！点击地图上的标记可以查看详情哦~';
    }
    
    if (lowerMsg.includes('历史') || lowerMsg.includes('古老')) {
        return '最古老的桥是赵州桥，建于隋朝(605-618年)，已经有1400多年历史了，是世界上现存最早的敞肩式石拱桥！';
    }
    
    if (lowerMsg.includes('你好') || lowerMsg.includes('hi') || lowerMsg.includes('hello')) {
        return '你好呀！我是桥宝，你的智能导游~ 可以问我任何关于中国桥梁的问题，或者说「看看XX桥」来查看桥梁详情哦！';
    }
    
    if (lowerMsg.includes('谢谢')) {
        return '不客气！很高兴能帮到你~ 还有什么想了解的吗？';
    }
    
    if (lowerMsg.includes('北京')) {
        return '北京有著名的卢沟桥，位于北京市丰台区，是「卢沟晓月」景观的所在地，桥上的石狮子非常有名！';
    }
    
    if (lowerMsg.includes('河北')) {
        return '河北有著名的赵州桥，又名安济桥，位于石家庄市赵县，是隋朝李春设计的，已有1400多年历史！';
    }
    
    if (lowerMsg.includes('广东') || lowerMsg.includes('珠海')) {
        return '广东有著名的港珠澳大桥，是世界上最长的跨海大桥！还有广州的海珠桥也很出名~';
    }
    
    return '我是桥宝，你的智能导游~ 可以问我关于中国著名桥梁的问题，比如「最长的大桥」、「古老的桥」或者直接说「看看卢沟桥」等，我帮你跳转查看详情！';
}

function determineAction(userMessage, bridges) {
    const lowerMsg = userMessage.toLowerCase();
    
    for (const bridge of bridges) {
        const name = bridge.name.toLowerCase();
        const nameEn = bridge.name_en ? bridge.name_en.toLowerCase() : '';
        
        if (lowerMsg.includes(name) || lowerMsg.includes(nameEn) || 
            lowerMsg.includes(bridge.province) || lowerMsg.includes(bridge.city)) {
            return {
                type: 'bridge',
                bridgeId: bridge.id,
                bridgeName: bridge.name,
                lat: bridge.latitude,
                lng: bridge.longitude
            };
        }
    }
    
    return { type: 'none' };
}

// ============ 用户认证API ============

app.post('/api/register', (req, res) => {
    const { username, email, password } = req.body;

    if (!username || !email || !password) {
        return res.status(400).json({ message: '请填写所有字段' });
    }

    const query = 'INSERT INTO users (username, email, password) VALUES (?, ?, ?)';
    db.query(query, [username, email, password], (err, result) => {
        if (err) {
            if (err.code === 'ER_DUP_ENTRY') {
                return res.status(409).json({ message: '用户名或邮箱已存在' });
            }
            return res.status(500).json({ message: '注册失败，服务器错误' });
        }
        res.status(201).json({ message: '注册成功' });
    });
});

app.post('/api/login', (req, res) => {
    const { identifier, password } = req.body;

    if (!identifier || !password) {
        return res.status(400).json({ message: '请输入账号和密码' });
    }

    const query = 'SELECT * FROM users WHERE (username = ? OR email = ?) AND password = ?';
    db.query(query, [identifier, identifier, password], (err, results) => {
        if (err) return res.status(500).json({ message: '登录失败，服务器错误' });

        if (results.length > 0) {
            const user = results[0];
            res.json({
                message: '登录成功',
                user: { id: user.id, username: user.username, email: user.email }
            });
        } else {
            res.status(401).json({ message: '账号或密码错误' });
        }
    });
});

// Start Server
async function startServer() {
    try {
        await initDatabase();
        
        app.listen(PORT, () => {
            console.log(`桥韵中华服务器启动成功!`);
            console.log(`网址: http://localhost:${PORT}`);
        });
    } catch (err) {
        console.error('启动失败:', err);
        process.exit(1);
    }
}

startServer();
